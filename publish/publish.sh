...
function build_mimic() {
  pushd $MIMIC_SRC/mimic
  make clean
  ./configure --prefix=${MIMIC_ARTIFACT_DIR}/${MIMIC_SYSTEM_TARGET} ${MIMIC_CONFIG_OPTTIONS}
...
echo ${VERSION} > ${TOP}/dist/mimic/latest
_run s3cmd -c ${HOME}/.s3cfg.mycroft-artifact-writer put --acl-public ${TOP}/dist/latest s3://bootstrap.mycroft.ai/artifacts/${ARCH}/mimic/latest
